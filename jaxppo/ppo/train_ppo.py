...
def init_agent(  # pylint: disable=W0102, R0913
    ...
    env_args = EnvironmentProperties(env, env_params, num_envs, continuous)
    network_args = NetworkProperties(
        actor_architecture, critic_architecture, lstm_hidden_size
    )
...
def make_train(  # pylint: disable=W0102, R0913
    ...
    env, env_params, env_id, continuous = prepare_env(
        env_id, episode_length, env_params=env_params, num_envs=num_envs
    )
    ...
    def train(
        ...
        reset_rng = (
            jax.random.split(reset_key, num_envs) if mode == "gymnax" else reset_key
        )

        obsv, env_state = reset_env(reset_rng, env, mode, env_params)
        # TRAIN LOOP
        ...
        recurrent = (
            actor_state.hidden_state is not None
            and critic_state.hidden_state is not None
        )
        ...
        def _update_step_pre_partial(  # pylint: disable=R0913,R0914
            ...
            def _env_step_pre_partial(
                ...
                pi, new_actor_hidden_state = get_pi(
                    runner_state.actor_state.state,
                    runner_state.actor_state.state.params,
                    (
                        runner_state.last_obs[jnp.newaxis, :]
                        if recurrent
                        else runner_state.last_obs
                    ),
                    runner_state.actor_state.hidden_state if recurrent else None,
                    runner_state.last_done[jnp.newaxis, :] if recurrent else None,  # type: ignore[index]
                    recurrent,
                )
                value, new_critic_hidden_state = predict_value(
                    runner_state.critic_state.state,
                    runner_state.critic_state.state.params,
                    (
                        runner_state.last_obs[jnp.newaxis, :]
                        if recurrent
                        else runner_state.last_obs
                    ),
                    runner_state.critic_state.hidden_state if recurrent else None,
                    runner_state.last_done[jnp.newaxis, :] if recurrent else None,  # type: ignore[index]
                    recurrent,
                )
                runner_state.critic_state.replace(hidden_state=new_critic_hidden_state)
                runner_state.actor_state.replace(hidden_state=new_actor_hidden_state)
                ...
                obsv, env_state, reward, done, info = step_env(
                    rng_step, runner_state.env_state, action, env, mode, env_params
                )
                transition = Transition(
            ...
            initial_actor_hidden_state = runner_state.actor_state.hidden_state
            initial_critic_hidden_state = runner_state.critic_state.hidden_state
            if recurrent:
                if (
                ...
                def _update_minbatch_pre_partial(  # pylint: disable=R0914
                    ...
                    (critic_loss, (update_targets, values)), critic_grads = (
                        ppo_critic_loss_grad_function(
                            critic_state.state.params, critic_state.state, batch_info
                        )
                    )
                    new_actor_state = actor_state.state.apply_gradients(
                        grads=actor_grads
                    )
                    new_critic_state = critic_state.state.apply_gradients(
                        grads=critic_grads
                    )
                    actor_state = MaybeRecurrentTrainState(
                        new_actor_state, actor_state.hidden_state
                    )
                    critic_state = MaybeRecurrentTrainState(
                        new_critic_state, critic_state.hidden_state
                    )
                    total_loss = None
                    metrics = (
                        total_loss,
